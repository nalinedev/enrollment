...
public class CooperativeUserGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all cooperativeUsers").get("/api/cooperative-users").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new cooperativeUser")
                        .post("/api/cooperative-users")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_cooperativeUser_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created cooperativeUser").get("${new_cooperativeUser_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created cooperativeUser").delete("${new_cooperativeUser_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the CooperativeUser entity").exec(scn);
