 ...
 */
public class SocialProfileGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all socialProfiles").get("/api/social-profiles").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new socialProfile")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                    ...
                                    ", \"educationLevel\": \"SAMPLE_TEXT\"" +
                                    ", \"housingStatus\": \"SAMPLE_TEXT\"" +
                                    ", \"residenceSince\": \"2020-01-01T00:00:00.000Z\"" +
                                    ", \"disabilityStatus\": null" +
                                    ", \"disabilityDescription\": null" +
                                    ", \"socialCategory\": \"SAMPLE_TEXT\"" +
                                    ", \"notes\": null" +
                                    "}"
                            )
                        )
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created socialProfile").get("${new_socialProfile_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created socialProfile").delete("${new_socialProfile_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SocialProfile entity").exec(scn);
