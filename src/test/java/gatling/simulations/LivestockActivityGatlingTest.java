...
public class LivestockActivityGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all livestockActivities")
                    .get("/api/livestock-activities")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new livestockActivity")
                        .post("/api/livestock-activities")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                    ...
                                    ", \"monthlyRevenue\": 0" +
                                    ", \"employees\": 0" +
                                    ", \"veterinaryServiceAvailable\": null" +
                                    ", \"feedSource\": \"SAMPLE_TEXT\"" +
                                    ", \"waterSource\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_livestockActivity_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created livestockActivity").get("${new_livestockActivity_url}").headers(headersHttpAuthenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created livestockActivity").delete("${new_livestockActivity_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the LivestockActivity entity").exec(scn);
