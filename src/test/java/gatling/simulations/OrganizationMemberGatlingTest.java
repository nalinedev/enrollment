...
public class OrganizationMemberGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all organizationMembers")
                ...
                .exec(
                    http("Create new organizationMember")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_organizationMember_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created organizationMember").get("${new_organizationMember_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created organizationMember").delete("${new_organizationMember_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the OrganizationMember entity").exec(scn);
