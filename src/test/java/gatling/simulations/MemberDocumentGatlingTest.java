...
public class MemberDocumentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all memberDocuments").get("/api/member-documents").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new memberDocument")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                    ...
                                    ", \"originalFileName\": \"SAMPLE_TEXT\"" +
                                    ", \"storedFileName\": \"SAMPLE_TEXT\"" +
                                    ", \"contentType\": \"SAMPLE_TEXT\"" +
                                    ", \"fileSize\": 0" +
                                    ", \"storagePath\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_memberDocument_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created memberDocument").get("${new_memberDocument_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created memberDocument").delete("${new_memberDocument_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MemberDocument entity").exec(scn);
