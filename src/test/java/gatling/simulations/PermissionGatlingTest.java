...
public class PermissionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all permissions").get("/api/permissions").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created permission").get("${new_permission_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created permission").delete("${new_permission_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
