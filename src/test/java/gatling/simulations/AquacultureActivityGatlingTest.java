...
public class AquacultureActivityGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all aquacultureActivities")
                    .get("/api/aquaculture-activities")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new aquacultureActivity")
                        .post("/api/aquaculture-activities")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                    ...
                                    ", \"productionUnitDescription\": \"SAMPLE_TEXT\"" +
                                    ", \"status\": \"ACTIVE\"" +
                                    ", \"annualRevenue\": 0" +
                                    ", \"monthlyRevenue\": 0" +
                                    ", \"employees\": 0" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_aquacultureActivity_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created aquacultureActivity").get("${new_aquacultureActivity_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created aquacultureActivity").delete("${new_aquacultureActivity_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AquacultureActivity entity").exec(scn);
