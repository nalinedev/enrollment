...
public class AquaticSpeciesGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all aquaticSpecieses").get("/api/aquatic-species").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new aquaticSpecies")
                        .post("/api/aquatic-species")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_aquaticSpecies_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created aquaticSpecies").get("${new_aquaticSpecies_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created aquaticSpecies").delete("${new_aquaticSpecies_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AquaticSpecies entity").exec(scn);
