...
/**
 * Performance test for the AgriculturalProduction entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v9.2.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class AgriculturalProductionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all agriculturalProductions")
                    .get("/api/agricultural-productions")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new agriculturalProduction")
                        .post("/api/agricultural-productions")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                    "\"area\": 0" +
                                    ", \"areaUnit\": \"SAMPLE_TEXT\"" +
                                    ", \"plantingDate\": \"2020-01-01T00:00:00.000Z\"" +
                                    ", \"harvestStartDate\": \"2020-01-01T00:00:00.000Z\"" +
                                    ", \"harvestEndDate\": \"2020-01-01T00:00:00.000Z\"" +
                                    ", \"productionQuantity\": 0" +
                                    ", \"productionUnit\": \"SAMPLE_TEXT\"" +
                                    ", \"expectedAnnualProduction\": 0" +
                                    ", \"numberOfPlants\": 0" +
                                    ", \"plantingDensity\": 0" +
                                    ", \"productionYear\": 0" +
                                    ", \"status\": \"PLANNED\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_agriculturalProduction_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created agriculturalProduction")
                            .get("${new_agriculturalProduction_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created agriculturalProduction")
                        .delete("${new_agriculturalProduction_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AgriculturalProduction entity").exec(scn);
