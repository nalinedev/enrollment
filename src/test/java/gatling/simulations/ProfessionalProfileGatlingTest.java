...
public class ProfessionalProfileGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all professionalProfiles")
                ...
                .exec(
                    http("Create new professionalProfile")
                        .post("/api/professional-profiles")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                    "\"employmentStatus\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_professionalProfile_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created professionalProfile").get("${new_professionalProfile_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created professionalProfile").delete("${new_professionalProfile_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ProfessionalProfile entity").exec(scn);
