...
public class IdentityDocumentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all identityDocuments").get("/api/identity-documents").headers(headersHttpAuthenticated).check(status().is(200)))
                        ...
                        .body(
                            StringBody(
                                "{" +
                                    ...
                                    ", \"issuingCountry\": \"SAMPLE_TEXT\"" +
                                    ", \"status\": \"VALID\"" +
                                    ", \"verified\": null" +
                                    ", \"verificationDate\": \"2020-01-01T00:00:00.000Z\"" +
                                    ", \"verificationComment\": \"SAMPLE_TEXT\"" +
                                    "}"
                            )
                        )
                        .asJson()
                ...
                .repeat(5)
                .on(
                    exec(http("Get created identityDocument").get("${new_identityDocument_url}").headers(headersHttpAuthenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created identityDocument").delete("${new_identityDocument_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
