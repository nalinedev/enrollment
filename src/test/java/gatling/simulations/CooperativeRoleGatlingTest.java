...
public class CooperativeRoleGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all cooperativeRoles").get("/api/cooperative-roles").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new cooperativeRole")
                        .post("/api/cooperative-roles")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_cooperativeRole_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created cooperativeRole").get("${new_cooperativeRole_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created cooperativeRole").delete("${new_cooperativeRole_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the CooperativeRole entity").exec(scn);
