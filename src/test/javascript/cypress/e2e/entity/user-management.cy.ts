...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'Brandon_Torp-Medhurst', email: 'Kristopher_Von10@hotmail.com' };
  ...
  describe.skip('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('Kitty20');
      cy.get(`[data-cy="login"]`).should('have.value', 'Kitty20');

      cy.get(`[data-cy="firstName"]`).type('Pete');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Pete');

      cy.get(`[data-cy="lastName"]`).type('Larson');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Larson');

      cy.get(`[data-cy="email"]`).type('Gregg31@gmail.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Gregg31@gmail.com');
